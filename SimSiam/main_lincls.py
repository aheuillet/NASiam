...
from simsiam.builder import convert_cifar
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == "resnet18":
        ...
        model = convert_cifar(models.__dict__[args.arch], num_classes, zero_init=False)
    else:
    ...
    if args.dataset == "imagenet":
        ...
        val_dataset = datasets.CIFAR100(root=args.data, train=False, download=True, transform=valid_transform)
    ...
    writer = SummaryWriter(f'./runs/lincls/lincls-{args.dataset}-{time.strftime("%Y%m%d-%H%M%S")}')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        writer.add_scalar('val_acc', acc1, global_step=epoch)

        # remember best acc@1 and save checkpoint
        is_best = acc1 > best_acc1
        best_acc1 = max(acc1, best_acc1)

        print(f"Best Val Acc@1: {best_acc1}")
...
def sanity_check(state_dict, pretrained_weights):
    ...
    for k in list(state_dict.keys()):
        ...
        k_pre = 'encoder.' + k #'module.encoder.' + k[len('module.'):] if k.startswith('module.') else 'module.encoder.' + k
