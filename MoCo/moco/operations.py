...
Genotype = namedtuple('Genotype', 'encoder')
...
def linear_wrapper(in_dim, out_dim, act=None):
  act = nn.ReLU(inplace=True) if act == None else act
  if in_dim < out_dim:
      return nn.Linear(in_dim, out_dim)
  else:
      return nn.Sequential(nn.Linear(in_dim, out_dim), nn.BatchNorm1d(in_dim, out_dim), act)

def op_wrapper(in_dim, out_dim, op, act=None):
  act = nn.ReLU(inplace=True) if act == None else act
  if in_dim > out_dim:
