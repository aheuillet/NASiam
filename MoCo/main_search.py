...
parser.add_argument('--search_lr', default=3e-4, type=float,
...
parser.add_argument('--search_wd', default=1e-3, type=float,
                     help='search weight decay (default: 1e-3)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    search_optimizer = torch.optim.Adam(model.arch_parameters(),
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            model.cell_encoder.alphas = alphas
    ...
    if args.dataset == "imagenet":
        ...
        #val_dataset = datasets.ImageNet(args.data, split="val", transform=moco.loader.TwoCropsTransform(transforms.Compose(augmentation)))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, val_loader, model, criterion, optimizer, search_optimizer, epoch, args)
    ...
    alphas = model.arch_parameters()[0]
    genotype = parse_genotype(torch.softmax(alphas, dim=1).data.cpu().numpy(), "./genotypes", "sparse", model_name=f"{args.dataset}_{args.arch}")

def search_step(val_loader, model, criterion, search_optimizer, args):
    ...
    loss = model(im_q=input_search[0], im_k=input_search[1])
...
def train(train_loader, val_loader, model, criterion, optimizer, search_optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        search_step(val_loader, model, criterion, search_optimizer, args)
